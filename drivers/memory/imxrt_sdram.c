...
static int __maybe_unused imxrt_semc_resume(struct device *dev){
    return 0;
}
static int __maybe_unused imxrt_semc_suspend(struct device *dev){
    return 0;
}


static SIMPLE_DEV_PM_OPS(imxrt_semc_pm_ops, imxrt_semc_suspend,
			 imxrt_semc_resume);

static const struct of_device_id imxrt_semc_match[] = {
	{.compatible = "fsl,imxrt-semc"},
	{}
};
MODULE_DEVICE_TABLE(of, imxrt_semc_match);

static struct platform_driver imxrt_semc_driver = {
	.probe	= imxrt_semc_probe,
	.remove	= imxrt_semc_remove,
	.driver	= {
		.name = "imxrt_semc",
		.of_match_table = imxrt_semc_match,
		.pm = &imxrt_semc_pm_ops,
	},
};
module_platform_driver(imxrt_semc_driver);
